class Node():
    """
    Нода с числовым значением и с ссылкой на следующиую ноду
    """
...
class Stack():
    """
    Стак, использующий ноды с числовым значением и с сылкой на следующиую ноду
    Также есть две вспомогательных ссылки на первую и последнию ноду стака: head и tail

    Метод min() работает за константу. Для этого существует стак минимумов. Метод min() возращает последний елемент в стаке минимумов
    Когда добавляется новый елемент, если он меньше текущего минимума, то он добавляется в стак минимумов.
    Когда уберается елемент, который является текущем минимумом, то он тоже уберается из стака минимумов.

    :param has_min: Если правда, присуствует функционал метода min(). Это нужно для стака минимума чтобы избежать рекурсию
    """
